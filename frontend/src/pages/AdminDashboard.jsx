  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {posts.length === 0 ? (
        ...
        posts.map((post) => (
          <div key={post._id} className="bg-white p-4 rounded shadow mb-6">
            <div className="flex justify-between">
              <div>
                <p className="font-semibold">{post.user?.name || "Unknown User"}</p>
                <p className="text-sm text-gray-500">{post.location} | {post.domain}</p>
              </div>
              <span className={`text-xs px-2 py-1 rounded ${post.status === "complete" ? "bg-green-500" : "bg-yellow-500"} text-white`}>
                {post.status}
              </span>
            </div>
            ...
            >
              <input
                ...
                className="border p-1 rounded w-2/3 mr-2"
              />
              <button type="submit" className="bg-gray-700 text-white px-2 py-1 rounded">
                Comment
              </button>
            </form>
